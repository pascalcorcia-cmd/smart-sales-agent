...
const MODULES = [
  ...
  { id: 'pricing', label: 'Négociation Prix', icon: '💰', skill: 'pricing-negotiation', desc: 'Contre-arguments et marges de manœuvre' },
...
export default function AccountPlan() {
  ...
  const buildPrompt = (mod) => {
    ...
    const prompts = {
      ...
      competitors: `${baseContext}${filesContext}\n${previousResults}\nAnalyse concurrentielle pour ${companyName} (${companyUrl}). Exploite les documents fournis pour identifier les concurrents mentionnés. Détecte les solutions actuellement utilisées, évalue les coûts de switching, crée des battle cards pour chaque concurrent détecté. Puis passe du constat à l'action : pour chaque concurrent identifié, donne un angle de positionnement actif — le message précis à utiliser face à ce concurrent (pas une simple liste de forces/faiblesses), les questions qui font ressortir tes avantages, et comment répondre si le prospect dit déjà utiliser ou évaluer cette solution.`,
      ...
      brief: `${baseContext}${filesContext}\n${previousResults}\nRédige un brief express tenant sur une seule page pour ${companyName} avant un rendez-vous. Va à l'essentiel : qui est l'entreprise en 2-3 lignes, les 3 informations les plus importantes à connaître, les 2-3 questions clés à poser, un piège à éviter. Pas de sections détaillées, pas de remplissage — c'est un pense-bête à lire en 2 minutes juste avant d'entrer en réunion, pas un rapport.`,
      ...
      pricing: `${baseContext}${filesContext}\n${previousResults}\nPrépare la négociation de prix pour ${companyName}. Identifie la marge de manœuvre probable (à partir des infos disponibles : taille du deal, budget évoqué, urgence). Anticipe les demandes de rabais et objections de prix les plus probables (comparaison concurrent moins cher, budget serré, "on peut avoir mieux ailleurs", demande de remise volume) avec une réponse pour chacune qui protège la valeur perçue plutôt que de céder direct sur le prix. Propose 2-3 leviers non-monétaires à offrir en échange d'une concession (durée d'engagement, volume, référence client, délai de paiement) et une ligne rouge : le prix plancher en dessous duquel ne pas descendre et pourquoi.`,
